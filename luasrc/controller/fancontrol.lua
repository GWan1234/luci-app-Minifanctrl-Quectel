...
function index()
	entry({"admin", "system", "fancontrol"}, template("Minifanctrl/fancontrol"), _("风扇控制"), 90)
...
function action_fansttp()
    ...
    if conf_file then
        ...
        if config:match("模组温度") then
			fansv="模组温度"
            local sendat_command = io.popen("sendat 2 'AT+QTEMP' | grep 'QTEMP' | sed -n '$p' | cut -d, -f2 | sed '/^$/d' | tr -d '\"'")
